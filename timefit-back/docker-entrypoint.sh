...
if [ -f "$ACCESS_PRIVATE" ] && [ -f "$REFRESH_PRIVATE" ]; then
    ...
    openssl pkey -in "$ACCESS_PRIVATE" -pubout -out "$KEYS_DIR/access_public_key.pem" 2>/dev/null

    # Refresh Token 키 생성 (PKCS#8 형식)
    openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:4096 -out "$REFRESH_PRIVATE" 2>/dev/null
    openssl pkey -in "$REFRESH_PRIVATE" -pubout -out "$KEYS_DIR/refresh_public_key.pem" 2>/dev/null
