...
@Schema(description = "영업시간 관리 Response DTO 모음")
public class OperatingHoursResponseDto {
    ...
    public record DaySchedule(
            @Schema(description = "요일 (0=일요일, 1=월요일, ..., 6=토요일)", example = "1")
            Integer dayOfWeek,

            @Schema(description = "영업 시작 시간", example = "09:00", nullable = true)
            String openTime,

            @Schema(description = "영업 종료 시간", example = "22:00", nullable = true)
            String closeTime,

            @Schema(description = "휴무일 여부", example = "false")
            Boolean isClosed,

            @Schema(description = "예약 가능 시간대 목록", nullable = true)
