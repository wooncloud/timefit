...
export default function ReservePage() {
  ...
  const [selectedDate, setSelectedDate] = useState<string>(weekDates[0].toISOString().split('T')[0]);
  ...
  return (
    <div className="flex flex-col bg-white">
      ...
      <div className="flex items-center gap-3 border-b px-4 py-3">
        <Link href={`/places/${params.id}`} className="flex h-10 w-10 items-center justify-center">
      ...
      <div className="flex-1 px-4 py-4">
        {/* 선택된 서비스 요약 */}
        <div className="mb-6">
          <h2 className="mb-3 text-sm font-semibold text-gray-900">선택한 서비스</h2>
          <div className="rounded-xl bg-gray-50 p-4">
            {selectedServices.map((service, index) => (
              <div
                key={service.id}
                className={cn('flex items-center justify-between', index > 0 && 'mt-3 border-t pt-3')}
              >
                <div>
            ...
            <div className="mt-4 flex items-center justify-between border-t pt-4">
              <p className="font-semibold text-gray-900">총 {totalDuration}분</p>
              <p className="text-lg font-bold text-[#3ec0c7]">{formatPrice(totalPrice)}</p>
            </div>
          </div>
        </div>
            ...
            {weekDates.map((date) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="mt-1 text-lg font-bold">{formatted.date}</span>
                  {isToday && (
                    <span
                      className={cn(
                        'mt-1 text-[10px]',
        ...
        <div className="mb-6">
          <div className="mb-3 flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-gray-900">요청사항 (선택)</h2>
          </div>
          <Textarea
